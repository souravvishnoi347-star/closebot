...
function StatusIcon({ status }: { status: Message["status"] }) {
  const iconClass = "h-3.5 w-3.5 text-white/90";
  switch (status) {
    case "sending":
    ...
    case "failed":
      return <XCircle className="h-3.5 w-3.5 text-red-200" />;
    default:
      return null;
  }
}
...
export function MessageBubble({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative rounded-2xl p-3.5 shadow-sm text-[15px] leading-relaxed",
          isAgent
            ? "rounded-br-sm bg-gradient-to-br from-sky-500 to-blue-600 text-white shadow-md"
      ...
      >
        ...
        >
          <span className={cn("text-[10px]", isAgent ? "text-white/80" : "text-slate-500")}>{time}</span>
