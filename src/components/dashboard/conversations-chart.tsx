  ...
  return (
    <section className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
      <header className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
        <div>
          <h2 className="text-[15px] font-bold text-slate-900">Conversations Over Time</h2>
          <p className="mt-1 text-xs font-medium text-slate-500">Daily message volume by direction</p>
        </div>
      ...
      <footer className="flex items-center gap-5 border-t border-slate-200 px-6 py-4 text-xs font-semibold text-slate-600">
...
function LineSvg({
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      {hovered && hover !== null && (
        <div
          className="pointer-events-none absolute top-0 z-10 -translate-x-1/2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[12px] shadow-lg"
          style={{ left: `${hover.tooltipLeftPx}px` }}
        >
          <div className="font-bold text-slate-900">{longDayLabel(hovered.day)}</div>
          <div className="mt-1 flex flex-col gap-1">
            <span className="flex items-center gap-2 font-medium text-blue-600">
              <span className="inline-block h-2 w-2 rounded-full bg-blue-500" />
              {hovered.incoming} incoming
            </span>
            <span className="flex items-center gap-2 font-medium text-violet-600">
