  ...
  return (
    <section className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
      <header className="border-b border-slate-200 px-6 py-5">
        <h2 className="text-[15px] font-bold text-slate-900">Pipeline Value</h2>
            ...
            <ul className="mt-5 space-y-2">
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="w-20 text-right font-bold text-slate-900 tabular-nums">
    ...
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label="Pipeline value by stage">
        ...
        <text
          ...
          y={cy + 14}
          textAnchor="middle"
          className="fill-slate-900 text-[18px] font-bold tabular-nums"
        >
